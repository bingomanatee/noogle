...
Collection.prototype.find = function() {
  ...
  if(len == 3){ // backwards compat for options object
    ...
    while(!is_option && idx < l) if(test[idx] in fields ){
      is_option = true;
      console.log('setting fields to options because it has ' + test[idx]);
    } else {
      idx++;
    }
  ...
  * disabling check - just do it right damnit!
  */
...
/*
  ...
  console.log(o.sort);
  console.log('calback');
  console.log(callback.toString().substring(0, 100));
*/

  callback(null, new Cursor(this.db, this, selector, fields, o.skip, o.limit, o.sort, o.hint, o.explain, o.snapshot, o.timeout)); 
};

Collection.prototype.find_p = function(params, callback) {
  var fields = selector = fields = skip = limit = sort = hint = explain = snapshot = timeout = undefined;
  var selector = {};
...
/*
  ...
  */
  ...
  callback(null, new Cursor(this.db, this, selector, fields, skip, limit, sort, hint, explain, snapshot, timeout));
};
